...
export class SyncDestinationMapper {
    /** Contains the actual remote path that is used for syncing. This
     * points to the exact sync destination including all the autogenerated
     * folders. Eg, is the remoteUri is /Repos/username/repoName, and we are
     * generating a folder inside it called "files" to sync to, we want to
     * keep that folder hidden from the ui. The actualRemoteUri will be
     * /Repos/username/repoName/files.
     */
    private get actualRemoteUri(): RemoteUri {
        return this.remoteUri.join("files");
    }
    constructor(
    ...
    localToRemote(localPath: LocalUri): RemoteUri {
        ...
        return new RemoteUri(
            path.posix.join(this.actualRemoteUri.path, relativePath)
        );
    }

    remoteToLocal(remotePath: RemoteUri): LocalUri {
        if (
            this.actualRemoteUri.path !== remotePath.path &&
            !this.actualRemoteUri.hasChild(remotePath)
        ) {
            throw new Error(
        ...
        const relative = path.relative(
            this.actualRemoteUri.localPath,
