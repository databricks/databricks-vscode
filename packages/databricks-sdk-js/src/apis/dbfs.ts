...
/**
 * This service allows users to interact with the Databricks File System (DBFS), which is a
 * distributed file system layer on top of S3. For example, users can create new files or list the
 * contents of a given path in DBFS.
 */
export class DbfsService {
    ...
    /**
     * Returns the contents of a file. If the file does not exist, this call will throw an exception
     * with ``RESOURCE_DOES_NOT_EXIST``. If the path is a directory, the read length is
     * negative, or if the offset is negative, this call will throw an exception with
     * ``INVALID_PARAMETER_VALUE``. If the read length exceeds 1 MB, this call will throw an
     * exception with ``MAX_READ_SIZE_EXCEEDED``. If ``offset + length`` exceeds the number of bytes in a
    ...
    /**
     * These test endpoints are replicas of the original endpoints used for Sisyphus rate limiting
     * integration testing
     */
    ...
    /**
     * Gets the file information of a file or directory. If the file or directory does not exist,
    ...
    /**
     * Lists the contents of a directory, or details of the file. If the file or directory does not
     * exist, this call will throw an exception with ``RESOURCE_DOES_NOT_EXIST``. Example of reply:
     *
     * .. code::
     *
     *     {
     ...
     * but can also be used as a convenient single call for data upload. Example usage:
     *
     * .. code::
     *
     *     curl -u USER:PASS -F contents=@localsrc -F path="PATH"
     *             https://XX.cloud.databricks.com/api/2.0/dbfs/put
     *
     * Please note that ``localsrc`` is the path to a local file to upload and this usage is
     * only supported with multipart form post (i.e. using -F or --form with curl).
     *
     ...
     *    -d '{"path":"PATH","contents":"BASE64"}' https://XX.cloud.databricks.com/api/2.0/dbfs/put``
     *
     * Amount of data that can be passed using contents (i.e. not streaming) parameter is limited to
     * 1 MB, ``MAX_BLOCK_SIZE_EXCEEDED`` will be thrown if exceeded. Please use streaming upload if you
     * want to upload large files, see :ref:`dbfsDbfsServicecreate`, :ref:`dbfsDbfsServiceaddBlock`
     * and :ref:`dbfsDbfsServiceclose` for details.
     */
    ...
    /**
     * Creates the given directory and necessary parent directories if they do not exist. If there
     * exists a file (not a directory) at any prefix of the input path, this call will throw an
     * exception with ``RESOURCE_ALREADY_EXISTS``. Note that if this operation fails it may
     ...
     * exist, this call will throw an exception with ``RESOURCE_DOES_NOT_EXIST``. If there
     * already exists a file in the destination path, this call will throw an exception with
     * ``RESOURCE_ALREADY_EXISTS``. If the given source path is a directory, this call will
     * always recursively move all files.
     */
     ...
     * call will throw an exception with ``IO_ERROR`` if the path is a non-empty directory
     ...
     * overwrite is set to false, this call will throw an exception with ``RESOURCE_ALREADY_EXISTS``. A
     ...
     * exist, this call will throw an exception with ``RESOURCE_DOES_NOT_EXIST``. If the block
    ...
    /**
     * Closes the stream specified by the input handle. If the handle does not exist, this call will
     * throw an exception with ``RESOURCE_DOES_NOT_EXIST``.
     */
